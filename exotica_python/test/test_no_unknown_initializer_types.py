    ...
    def test_unknown_initializer_types(self):
        ...
        if os.environ['ROS_PYTHON_VERSION'] == '2':
            output = subprocess.check_output(
                ["/usr/bin/env", "python", "-c", "'import pyexotica._pyexotica as exo ; initializers = exo.Setup.get_initializers()'"])
        elif os.environ['ROS_PYTHON_VERSION'] == '3':
            output = subprocess.check_output(
                ["/usr/bin/env", "python3", "-c", "'import pyexotica._pyexotica as exo ; initializers = exo.Setup.get_initializers()'"])
        else:
